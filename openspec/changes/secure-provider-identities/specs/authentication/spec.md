...
**Reason**: Replace characterized password behavior with an explicit native-method policy.
**Migration**: Existing human Users with a local hash retain native login; external-only Users do not gain it.

### Requirement: google_identity_resolution_baseline
**Reason**: Email is not stable identity or proof of account ownership.
**Migration**: Reenroll legacy Google identities through an independently verified existing method or controlled operator enrollment. Never backfill by matching email.

### Requirement: google_browser_challenge_baseline
**Reason**: Provider-neutral hosted login requires durable single-use operations and validated subjects.
**Migration**: Invalidate outstanding legacy challenges and restart hosted login.

### Requirement: oauth_session_handoff_baseline
**Reason**: Handoff must respect server-side revocation as well as browser binding.
**Migration**: Invalidate legacy handoffs; the callback issues a new bound handoff.

### Requirement: session_validity_baseline
**Reason**: Logout and offboarding must revoke sessions permanently.
**Migration**: Reject all legacy stateless session tokens and require fresh authentication.
