...
The server already authenticates runtime navigation, computes the authorization-filtered app catalog, distinguishes navigation from assets, and returns package HTML. That delivery boundary is the one place which owns both the selected package presentation and the current member's allowed destinations.
...
For default presentation, the server transforms only the client entry HTML response by inserting a host-owned fixed header before the package body content and host-owned style/behavior before the document closes. It leaves package scripts, the mount root and relative asset URLs intact. Explicit full-screen HTML and every non-HTML asset pass through byte-for-byte.
...
Extensionless authenticated HTML navigation falls back to the declared client entry after confirming app access; requests with an asset extension never do. Browser history therefore keeps the deep path while relative and root-relative package assets retain existing behavior.

### Add one optional strict presentation declaration

The manifest schema gains `presentation: z.literal('fullscreen').optional()`. Absence is the shell default. The runtime package record carries the parsed value to HTML delivery and includes it in the immutable artifact identity already derived from package files.

A boolean was rejected because `true` has no durable vocabulary and makes future presentation modes ambiguous. Inferring full-screen from root markup or CSS was rejected because it makes presentation non-reviewable and non-validatable.
...
- [A package assumes the viewport starts at y=0 or sets fixed full-height content] → Reserve shell space with a body inset for default presentation; packages that truly require the full viewport must explicitly declare full-screen.
...
Ship the host and manifest parser together. Existing packages without a declaration intentionally move to shell presentation. Mark the independent `other` fixture full-screen to retain and prove the opt-out; Tasker remains undeclared and removes only its duplicate platform back link. Rollback restores the prior runtime HTML response and removes the optional declaration; there is no database state or data migration.
