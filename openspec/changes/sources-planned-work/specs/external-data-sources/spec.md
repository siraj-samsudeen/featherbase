...
WHEN a connected table's data source has dropped or retyped a column the
table maps, Featherbase SHALL fail that table's requests with a clear
message naming the column, rather than quietly dropping the field or
returning wrong data. A builder SHALL be able to see what changed at the
data source — added, removed or retyped columns — and choose which of those
changes to bring in.

#### Scenario: A mapped column disappears

- **WHEN** a data source drops a column a connected table maps, and someone
  opens that table
...
additions disappears or changes its id at the data source, Featherbase SHALL
keep a record of those orphaned additions and let a builder review them,
rather than losing track of them silently.
...
WHERE a connected table's data source has no last-changed time to check, a
...
### Requirement: Show the user what changed when a save is refused

WHEN a save on a connected table is refused because the row changed at the
data source since it was loaded, Featherbase SHALL show the user the row's
current values alongside what they tried to save, instead of only saying the
save was refused.

#### Scenario: Seeing what changed

- **WHEN** a person's save is refused because someone else changed the row
  first
- **THEN** they see the row's current values next to the values they tried
  to save, without having to reload and compare by hand
