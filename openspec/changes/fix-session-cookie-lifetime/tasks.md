...
- [x] 1.1 Add failing HTTP regressions for password, Google, and preview sign-in that configure distinct session lengths, assert each `sid` cookie's `Max-Age` matches its token lifetime, cover one hour and 720 hours, and preserve `HttpOnly`, `SameSite=Lax`, `Path=/`, and configured HTTPS `Secure` behavior; verify the focused tests fail on the fixed seven-day implementation.
