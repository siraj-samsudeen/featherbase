...
- [x] 1.2 Package the retained v1 behavior at the exact `dist/` paths in Dev's recorded 0.0.1 declaration and the independently built 2.1.0 artifact in versioned final-image roots, cite `@spec runtime_upgrade_recovery_boundary.packaged_upgrade_retains_exact_predecessor` at the deciding configuration and checking test, and verify the focused test passes.
...
- [x] 2.4 Retain exact Feather Dash 0.1.3 and active 0.2.1 artifacts and provider wiring in the final image; prove their reviewed digests and obtain independent review of the integration delta.
