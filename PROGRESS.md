...
absence/clear scope, not an update whitelist; and UPS-R5 says a keyless Import
Log records `updated: 0` while match-key configuration remains null. The server
assertion verifies the raw serialized `'0'` before its numeric zero. Evidence
text cites only existing/upheld proof; no new runtime claim or generated manual
content was added.
...
settled Oracle design requirements, after main included PRs #213, #261,
#265 and #263. Three journeys, eleven rules, three invariants and three
compound hazards cover native-only eligibility, atomic field merge,
conditional conflict review, receipt replay, ordinary locked lifecycle,
keyboard/accessibility behavior and in-memory draft/navigation safety.
All twenty obligations are `gap #259`; no Grid/server behavior or tests
were implemented, and stale prototype #187 was not revived.

Verified: `pnpm check:evidence` — 50 tests pass, 146 verdicts across eight
specs (65 gap, seven pinned, 71 proven, three rule-tier); `pnpm manual:build`
— existing import guide regenerated byte-identically, confirmed with
`git diff --exit-code -- docs/manual/spreadsheet-import.html`;
`git diff --check` passes. A local-link/obligation check confirmed twenty
unique Grid IDs, twenty gap verdicts and resolving spec/index links.
The current generator only supports spec 0008: no hand-authored Grid HTML
or fabricated screenshots were added. Next: independent spec review and
final focused Oracle review before merge; #259 remains open and eligibility
is the first implementation slice, not an already-proven capability.
...
and a duplicate step/derived screenshot slot, each naming the offending spec
line. The positive suffix mutation uses a valid `J1.1b` step under `IMP-J1`.

**Verified:** `node --test tools/*.test.mjs` (50 pass),
`node tools/check-evidence.mjs` (126 verdicts across 7 specs), and
`node tools/build-manual.mjs` (manual generated byte-identically; no output
diff).
