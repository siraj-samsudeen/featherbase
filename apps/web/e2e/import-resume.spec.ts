...
// (docs/testing/e2e-dsl-migration.md). `page.reload()` and every wizard
// control here stay inside named steps; `session.visit` carries navigation.
...
test('going to look at the imported rows and coming back resumes everything', async ({
  ...
  await session.step('open the wizard, drop the workbook, import sheet 1, plan sheet 2', async ({ page }) => {
  ...
  await session.step(
    'follow the result link away, and come back — nothing is lost',
    async ({ page }) => {
  ...
  await session.step('the resumed run finishes without re-importing what already landed', async ({ page }) => {
    await page.getByTestId('iw-import-one').click()
...
test('a full page reload resumes too', async ({ session, request }) => {
  const token = await adminToken(request)
  await session.visit('/admin')

  await session.step('drop the workbook, plan two sheets, then reload the whole page', async ({ page }) => {
    await page.getByTestId('import-data-link').click()
    await dropWorkbook(page)
    await page.getByTestId('iw-ov-master').check()
    await page.getByTestId('iw-ov-continue').click()
...
test('a finished import is not resumed — coming back starts the next one', async ({
  ...
  await session.step('finish a two-sheet import, then reload', async ({ page }) => {
    await page.getByTestId('import-data-link').click()
    await dropWorkbook(page)
...
test('starting over forgets the file and every choice about it', async ({ session }) => {
  await session.visit('/admin')

  await session.step('start over, and confirm it stays forgotten across a reload', async ({ page }) => {
    await page.getByTestId('import-data-link').click()
    await dropWorkbook(page)
    await page.getByTestId('iw-ov-master').check()
    await page.getByTestId('iw-ov-continue').click()
...
test('when the rows do not fit, the decisions still do', async ({ session }) => {
  await session.visit('/admin')

  await session.step(
    'simulate a quota-exceeded sessionStorage, plan sheet 1, and reload',
    async ({ page }) => {
      await page.getByTestId('import-data-link').click()
  ...
  await session.step('dropping the same file picks up exactly where it left off', async ({ page }) => {
