...
test('IMP-010: multi-sheet workbook — one sheet to a new Table, one mapped onto a renamed existing Table, dry-run first', async ({
  ...
  await session.step('drop the two-sheet workbook and select both on the overview', async ({ page }) => {
  ...
  await session.step('sheet 1 defaults to a new Table, Choice detected', async ({ page }) => {
    ...
    // shift them again (#94).
    await expect(page.getByTestId('iw-new-grid-0').locator('tbody tr').first()).toHaveAttribute(
      'data-testid',
      'iw-row-id-0',
    )
  ...
  await session.step('step to sheet 2, matched by columns onto the existing Table', async ({ page }) => {
  ...
  await session.step('dry-run catches the bad Int row before anything is written', async ({ page }) => {
  ...
  await session.step('import (skipping the bad row); both sheets land', async ({ page }) => {
...
test('IMP-013: leave a sheet out, drop a column, and drive the target picker', async ({
  ...
  const drop = XLSX.utils.aoa_to_sheet([['Drop Only'], ['never imported']])
  const wb = XLSX.utils.book_new()
  ...
  await session.visit('/admin')
  await session.step('drop the workbook and leave the second sheet unselected', async ({ page }) => {
    await page.getByTestId('import-data-link').click()
    await page.getByTestId('iw-file-input').setInputFiles({
  ...
  await session.step('the target picker searches and pins New Table…', async ({ page }) => {
  ...
  await session.step('drop the middle column and import just the 2 remaining rows', async ({ page }) => {
...
test('IMP-010: the list view Import button preselects that Table as the target', async ({
  ...
  await session.visit(`/admin/${encodeURIComponent(EXISTING_DT)}`)
  ...
  await session.step('open the Import button from the list, preselected to this Table', async ({ page }) => {
