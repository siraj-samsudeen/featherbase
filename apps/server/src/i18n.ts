...
export async function getCatalog(language: string): Promise<Record<string, string>> {
  ...
  const [ok] = await sql`select 1 from information_schema.tables where table_name = 'translation'`
  if (!ok) return {}
  const rows = await sql`
    select source_text, translated_text from translation where language = ${language}`
