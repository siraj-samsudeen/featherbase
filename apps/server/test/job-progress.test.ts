...
describe('JOB-005: job progress reporting', () => {
  it('delivers setProgress calls as job_progress events on the owner channel', async () => {
    const events: { channel: string; event: string; payload: unknown }[] = []
    ...
    await enqueue('progress_probe', {})
    await drainJobs()
    off()

    const progress = events.filter((e) => e.event === 'job_progress')
    expect(progress.length).toBe(3)
    // All target the owner's user channel.
    expect(progress.every((e) => e.channel === 'user:Administrator')).toBe(true)
    const percents = progress.map((e) => (e.payload as { percent: number }).percent)
    expect(percents).toEqual([0, 50, 100])
    expect((progress[2].payload as { message: string }).message).toBe('done')
  })
  ...
  it('clamps and rounds reported percentages', async () => {
    ...
    await enqueue('progress_clamp', {})
    await drainJobs()
    off()
    const percents = events.filter((e) => e.event === 'job_progress').map((e) => (e.payload as { percent: number }).percent)
