...
import { resolveDoctypeName } from '../src/meta'
import type { TestClient } from 'feather-testing-postgres'

// #56: /api/resource accepts slug spellings of DocType names with spaces —
...
describe('#56: DocType slugs in resource URLs', () => {
  ...
  // Two table-backed DocTypes can never share a slug — tableName() collapses
  // the same variants, so the second create collides on the table. Singles
  // (issingle) get no table, so they are the one way two names can normalize
  // alike — which is exactly the case the resolver must refuse to guess on.
  test('an exact match always wins over a slug match', async ({ admin }) => {
    await admin.post('/api/doctype', { name: 'Case Twin', issingle: true, fields: [{ fieldname: 'a', fieldtype: 'Data' }] })
    await admin.post('/api/doctype', { name: 'CASE Twin', issingle: true, fields: [{ fieldname: 'b', fieldtype: 'Data' }] })
    // Both spellings slug-match both DocTypes, but each IS an exact name —
    // so each resolves to itself instead of erroring as ambiguous.
    expect(await resolveDoctypeName('Case Twin')).toBe('Case Twin')
    expect(await resolveDoctypeName('CASE Twin')).toBe('CASE Twin')
  })

  test('an ambiguous slug is an error, not a silent pick', async ({ admin }) => {
    await admin.post('/api/doctype', { name: 'Twin Case', issingle: true, fields: [{ fieldname: 'a', fieldtype: 'Data' }] })
    await admin.post('/api/doctype', { name: 'TWIN CASE', issingle: true, fields: [{ fieldname: 'b', fieldtype: 'Data' }] })
  ...
  test('the slug map is invalidated with the meta cache', async ({ admin }) => {
    // Build the slug map before the DocType exists…
    ...
    await admin.post('/api/doctype', { name: 'Inval Note', fields: [{ fieldname: 'a', fieldtype: 'Data' }] })
