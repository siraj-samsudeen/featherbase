...
describe('system flag: rejected on the public Table API', () => {
  test('POST /api/doctype with system: true is a field-wise 417', async ({ admin }) => {
    await expect(
      admin.post('/api/doctype', { name: 'Sys Claim A', system: true, columns: COLUMNS }),
  ...
  test('PUT /api/doctype/:name cannot escalate an existing table to system', async ({ admin }) => {
    await admin.post('/api/doctype', { name: 'Sys Claim B', columns: COLUMNS })
    await expect(
      admin.put('/api/doctype/Sys%20Claim%20B', { system: true, columns: COLUMNS }),
  ...
  test('an API-created table lands as system = false and keeps its module', async ({ admin }) => {
    const meta = await admin.post<{ system: boolean; module: string }>('/api/doctype', {
      name: 'Sys Claim C',
      module: 'Custom',
      columns: COLUMNS,
    })
