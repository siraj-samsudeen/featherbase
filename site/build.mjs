 ...
 * ------------------------------------------------------------------ */
const cats = {
  metadata: { title: 'Metadata engine', short: 'DocTypes as data; tables, validation and meta generated from JSON.',
    files: ['apps/server/src/doctype-engine.ts', 'apps/server/src/meta.ts'],
    how: 'A DocType is rows in tab_doctype/tab_docfield. Saving one runs createTableDDL in doctype-engine.ts — a real Postgres table with the standard columns — and Zod validators are derived from the same metadata, so schema, validation and UI all share one source. meta.ts caches the processed definition and invalidates on change.' },
  ...
  'rest-api': { title: 'REST API', short: 'Frappe-wire-compatible resource + RPC endpoints, auth, tokens.',
    files: ['apps/server/src/index.ts', 'apps/server/src/methods.ts', 'apps/server/src/auth.ts'],
    how: 'index.ts mounts /api/resource/:doctype CRUD and /api/method/* RPC in Frappe’s wire shapes (sid cookie + Bearer token, exc_type error bodies, frappe.client.* namespace) so existing Frappe clients work unchanged. methods.ts holds the RPC registry apps can extend or override.' },
  permissions: { title: 'Permissions', short: 'Role DocPerms, permlevels, if_owner, user permissions — plus Postgres RLS.',
    files: ['apps/server/src/permissions.ts', 'apps/server/migrations/0010_rls.sql'],
    how: 'permissions.ts evaluates DocPerm rows (role × right × permlevel), if_owner scoping and user-permission link filters on every read and write; list queries get the same restrictions compiled into WHERE clauses. Underneath, migration 0010 adds real Postgres RLS for the desk_client role as defense in depth.' },
  'desk-ui': { title: 'Desk UI', short: 'One generic ListView and FormView render every DocType from metadata.',
    files: ['apps/web/src/pages/DeskLayout.tsx', 'apps/web/src/components', 'apps/web/src/pages/DocTypeBuilder.tsx'],
    how: 'The Desk is deliberately generic: a single ListView and FormView read the DocType metadata and render any model — fields, link typeaheads, child grids, section layout — with zero per-model frontend code. DeskLayout provides the shell (sidebar grouped by module, awesomebar); DocTypeBuilder edits DocTypes themselves.' },
  ...
  customization: { title: 'Customization', short: 'Custom fields, property setters, sandboxed server scripts.',
    files: ['apps/server/src/custom-fields.ts', 'apps/server/src/customizations.ts', 'apps/server/src/server-scripts.ts'],
    how: 'Site-level changes layer on top of shipped metadata instead of editing it: custom-fields.ts adds real columns at runtime, customizations.ts stores property overrides, and server-scripts.ts runs site-authored logic in a sandbox — the seed of the upgrade-safe overlay the design doc formalizes as D13.' },
...
const lifecycle = [
  'A client calls save_doc over REST — never Postgres directly (API-001).',
  'Auth resolves the session or token to a user and their roles.',
  'Permissions gate the doctype, the permlevels, and if_owner scope (PERM rows).',
  'Zod validation, generated from the DocType metadata, checks the payload field-wise (META-013).',
  'The lifecycle chain runs: validate → before_save hooks in registered controllers (DOC-003).',
  'One transaction writes parent + child tables, naming series, link checks (DOC-001, DOC-005).',
  'after_save fans out: versions, audit, background jobs, webhooks, realtime push (DOC-009).',
  'The generic Desk re-renders from the same metadata that defined all of the above (UI-002, UI-004).',
];
const milestones = [
