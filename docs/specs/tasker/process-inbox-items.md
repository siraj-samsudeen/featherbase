...
> evidence: rule-tier — server and component tests prove placement assigns the owner; preserving an In progress state lacks a dedicated asymmetric test.
...
> evidence: gap — project and Personal placement work, but Tasker has no guided Save and next or Skip loop yet.
